...
CREATE TABLE animal_bulk_update_operation (
    id UUID PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES app_user(id) ON DELETE CASCADE,
...
CREATE INDEX ix_animal_bulk_update_operation_owner
    ON animal_bulk_update_operation(user_id, farm_id, created_at DESC);

CREATE UNIQUE INDEX uq_movement_official_entry
...
CREATE UNIQUE INDEX uq_movement_official_exit
    ON movement_certificate(
        origin_livestock_id,
        UPPER(BTRIM(cod_remo)),
        UPPER(BTRIM(serie))
