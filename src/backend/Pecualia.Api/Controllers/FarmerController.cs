...
public static class FarmerController
{
    public static IEndpointRouteBuilder MapFarmerController(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapDelete("/{farmerId:long}", async (ClaimsPrincipal user, long farmerId, IFarmerService service, CancellationToken cancellationToken) =>
            await ControllerResults.ExecuteAsync(async () =>
            {
                await service.DeleteManagedFarmerAsync(user.GetUserId(), farmerId, cancellationToken);
                return new
                {
                    deleted = true
                };
